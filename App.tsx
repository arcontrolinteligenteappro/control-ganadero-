...
const App: React.FC = () => {
  ...
  // Reset subtab logic when main category changes
  useEffect(() => {
    ...
    <div className="flex p-1.5 bg-white dark:bg-[#0a0f0d] rounded-2xl border border-slate-200 dark:border-[#10b981]/20 shadow-sm mb-6 w-fit animate-in fade-in zoom-in-95">
      {items.map(item => (
        <button
          ...
          className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${subTab === item.id ? (theme === 'dark' ? 'bg-[#10b981] text-black shadow-[0_0_15px_rgba(16,185,129,0.4)]' : 'bg-slate-900 text-white shadow-lg') : 'text-slate-400 hover:text-slate-600 dark:hover:text-[#10b981]'}`}
        >
          {item.icon}
          {item.label}
        </button>
